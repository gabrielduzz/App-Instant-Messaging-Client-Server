...
public class ControllerEntrarGrupo implements Initializable {
  ...
  public static ControllerEntrarGrupo getControllerEntrarGrupo() {
  ...
  void entrarGrupo(MouseEvent event) throws Exception {
    ...
    for (Grupo grupo : Usuario.grupos) {
      if (grupo.getNome().equals(nomeGrupo)) {
        System.out.println("Voce ja esta nesse grupo");
        return;
      }
    }
    ...
    Usuario.clienteTCP.setMensagemAPDU(new MensagemAPDU("JOIN", nomeGrupo, Usuario.nome));
  ...
  private Parent createContentTelaChat() throws Exception {
    ...
    loader.setController(ControllerChat.getControllerChat());
